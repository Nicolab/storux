 ...
 * @return {string}
 */
export function generateStoreName(store) {
    return lcFirst(store.displayName || getFuncName(store.constructor));
}
export function getActionId(store, action) {
...
export function defineDisplayName(obj, displayName) {
 ...
 */
export function getProtoProps(obj, endProto = Object, props = []) {
    if (obj === endProto) {
        return props;
    }
    return getProtoProps(
    // @ts-ignore
    obj.__proto__, endProto, props.concat(Object
        // @ts-ignore
